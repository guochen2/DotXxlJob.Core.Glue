...
{
    public class PowerShellTaskExecutor : ITaskExecutor
    {
        IJobLogger _jobLogger;
        BaseGlueTaskExecutor _baseGlueTaskExecutor;

        public PowerShellTaskExecutor(IJobLogger jobLogger, BaseGlueTaskExecutor baseGlueTaskExecutor)
        {
        ...
        public async Task<ReturnT> Execute(TriggerParam triggerParam, CancellationToken cancellationToken)
        {
            try
            {
                _baseGlueTaskExecutor.Init("powershell", ".ps1", triggerParam);
